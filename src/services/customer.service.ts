...
export class CustomerService {
  ...
  public static createCustomer(data: CreateCustomerDTO, actor: ActorInfo): Customer {
    // 1. Role Permission Guard: OWNER and STAFF can create; ACCOUNTANT is forbidden
    if (actor.role === UserRole.ACCOUNTANT) {
      throw new Error('Accountants do not have permission to register new customers.');
    }
  ...
  public static updateCustomer(id: string, data: UpdateCustomerDTO, actor: ActorInfo): Customer {
    // 1. Role Permission Guard: OWNER and STAFF can edit; ACCOUNTANT is forbidden
    if (actor.role === UserRole.ACCOUNTANT) {
      throw new Error('Accountants do not have permission to modify customer profiles.');
    }
