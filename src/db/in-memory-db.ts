...
class InMemoryDatabase {
  ...
  public notifyListeners(): void {
    this.listeners.forEach((listener) => {
      try {
        ...
        console.error('dbRepository listener notification error', e);
      }
    });
  }
  ...
  // REMOTE REAL-TIME HYDRATION
  public setUsersFromRemote(remoteUsers: User[]): void {
    ...
    for (const seedUser of SEED_USERS) {
      ...
      if (existingIdx === -1) {
        ...
        // If remote has the user, ensure password fallback from seed is maintained if not in remote
        if (!merged[existingIdx].password && seedUser.password) {
          merged[existingIdx] = { ...seedUser, ...merged[existingIdx], password: seedUser.password };
        }
      }
    }
    this.users = merged;
  ...
  public setCustomersFromRemote(remoteCustomers: Customer[]): void {
    this.customers = [...remoteCustomers];
    this.notifyListeners();
  }

  public setTransactionsFromRemote(remoteTransactions: Transaction[]): void {
    this.transactions = [...remoteTransactions];
    this.notifyListeners();
  }

  public setPaymentsFromRemote(remotePayments: Payment[]): void {
    this.payments = [...remotePayments];
    this.notifyListeners();
  }

  public setLedgerEntriesFromRemote(remoteEntries: LedgerEntry[]): void {
    this.ledgerEntries = [...remoteEntries];
    this.notifyListeners();
  }

  public setWholesalersFromRemote(remoteWholesalers: Wholesaler[]): void {
    this.wholesalers = [...remoteWholesalers];
    this.notifyListeners();
  }

  public setInventoryMovementsFromRemote(remoteMovements: InventoryMovement[]): void {
    this.inventoryMovements = [...remoteMovements];
    this.notifyListeners();
  }

  public setRateConfigFromRemote(rates: RateConfiguration): void {
    this.rateConfig = { ...rates };
    this.notifyListeners();
  }
  ...
  public updateUserApprovalStatus(
    id: string,
    status: import('../types').UserApprovalStatus,
    reason?: string
  ): User | undefined {
  ...
  public saveTransaction(txn: Transaction): Transaction {
    ...
    if (existingIndex >= 0) {
      savedTxn = { ...txn, updatedAt: new Date().toISOString() };
      this.transactions[existingIndex] = savedTxn;
    } else {
      savedTxn = txn;
