...
export const RiceTradingView: React.FC<RiceTradingViewProps> = ({ onNavigate }) => {
  ...
  const submitSale = async () => {
    ...
    try {
      ...
      setMessage(`${sale.transaction.transactionNumber} saved. ${kg(sale.quantity)} rice removed from stock.`);
  ...
  return <div className="space-y-4 font-sans pb-10 max-w-6xl mx-auto">
    ...
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3"><Metric label="Today's Purchased" value={kg(purchases.filter((purchase) => new Date(purchase.transaction.date).toDateString() === new Date().toDateString()).reduce((sum, purchase) => sum + purchase.quantity, 0))} /><Metric label="Today's Sold" value={kg(sales.filter((sale) => new Date(sale.transaction.date).toDateString() === new Date().toDateString()).reduce((sum, sale) => sum + sale.quantity, 0))} /><Metric label="Current Rice Stock" value={kg(snapshot?.currentStock || 0)} /><Metric label="Receivables" value={money(sales.reduce((sum, sale) => sum + sale.amountDue, 0))} /></div>
    ...
    <div className="grid lg:grid-cols-2 gap-4"><section className="bg-white border border-stone-200 rounded-2xl overflow-hidden"><div className="p-4 border-b border-stone-100 flex justify-between"><h2 className="font-bold text-sm">Recent Rice Purchases</h2><ShoppingBag className="w-4 h-4 text-emerald-700" /></div>{purchases.slice(0, 12).map((purchase) => <div key={purchase.transaction.id} className="p-3 border-b border-stone-100 text-xs flex justify-between gap-3"><div><p className="font-semibold">{purchase.transaction.customerName || 'Customer'}</p><p className="text-stone-500">{purchase.transaction.transactionNumber} · {new Date(purchase.transaction.date).toLocaleDateString('en-IN')}</p></div><div className="text-right"><p className="font-bold">{kg(purchase.quantity)}</p><p className="text-stone-500">{money(purchase.purchaseRate)}/kg · {money(purchase.totalValue)}</p></div></div>)}</section><section className="bg-white border border-stone-200 rounded-2xl overflow-hidden"><div className="p-4 border-b border-stone-100 flex justify-between"><h2 className="font-bold text-sm">Wholesale Sales</h2><Truck className="w-4 h-4 text-sky-700" /></div>{sales.slice(0, 12).map((sale) => <div key={sale.transaction.id} className="p-3 border-b border-stone-100 text-xs flex justify-between gap-3"><div><p className="font-semibold">{sale.wholesaler.name}</p><p className="text-stone-500">{sale.transaction.transactionNumber} · {sale.transaction.paymentStatus}</p></div><div className="text-right"><p className="font-bold">{kg(sale.quantity)}</p><p className="text-stone-500">{money(sale.sellingRate)}/kg · {money(sale.saleValue)}</p></div></div>)}</section></div>
